...
class BindError:
    ...
    def __transfer_accident_data(self, tablename):
        ...
        self.__last_update < datetime.datetime.now() - datetime.timedelta(minutes=self.dleay_upd)):
            ...
            sql = "WITH temp as (SELECT n as tt from generate_series('"+str(start_update)+"'::timestamp,'"+str(end_update)+"'::timestamp,'"+str(self.__interval)+" minute'::interval) n ) \
INSERT  INTO  mvlab_"+str(tablename)+" (now_time, value) SELECT tt,(SELECT mode() WITHIN GROUP (ORDER BY value) as modevar FROM mvlab_temp_"+str(tablename)+" r WHERE  r.now_time>b.tt and r.now_time<=(b.tt+('"+str(self.__interval)+" minutes'::interval))) as value \
from mvlab_temp_"+str(tablename)+" a LEFT JOIN temp b ON a.now_time>b.tt and a.now_time<=(b.tt+('"+str(self.__interval)+" minutes'::interval)) WHERE a.value IS NOT NULL GROUP BY tt ORDER BY tt asc;"
            self._c.execute(sql)
            self._conn.commit()
            self._c.execute(
            ...
            self._conn.commit()
            sql_delete = "DELETE FROM mvlab_"+str(tablename)+" WHERE now_time>"+str(start_update)+" AND value is NULL;"
